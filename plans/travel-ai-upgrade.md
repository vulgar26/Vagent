...
- **主链路**：改写 →（可选）检索 →（可选）工具 → 上下文增强 → SSE 流式生成
...
- **E7 hashed membership**：已实现 `meta.retrieval_hit_id_hashes[]` + 配套 `meta.retrieval_*` 口径字段；请求须带 `X-Eval-Token`、`X-Eval-Target-Id`、`X-Eval-Dataset-Id`、`X-Eval-Case-Id`（与 `plans/eval-upgrade.md` 统一请求一致）。**SSOT**：`eval-upgrade.md` E7；代码：`D:\Projects\travel-ai-planner`（`RetrievalMembershipHasher`、`EvalMembershipHttpContext`、`EvalChatService`）。
- **与主链路差异**：当前 **SSE `TravelAgent`** 的阶段顺序/对外 `meta` 与评测口 **不必一致**；项目级 P0 若以「主产品」为准，须单独验收（见本文 §「控制流限制的可验收条款」与 `TravelAgent` 代码注释）。
...
- **固定线性阶段顺序**（不得动态分支成图）：`plan → retrieve(optional) → tool(optional) → write → guard`
  - `meta.step_count` 必须等于“实际执行过的阶段数”（仅串行阶段计数）
  - `meta` 中必须返回 `stage_order[]`（推荐枚举：`PLAN|RETRIEVE|TOOL|WRITE|GUARD`），用于回归验证“无跳转/无回环”
- **禁止动态 replan loop**：
...
- 先实现评测专用接口 `POST /api/v1/eval/chat`（`X-Eval-Token`），能返回 `answer/behavior/sources/tool/meta/latency_ms`。
...
- **实现**
  ...
  - 串行阶段（明确固定顺序，P0 不做动态分支）：`plan → retrieve(optional) → tool(optional) → write → guard`
  - **明确禁止（P0）**：
...
1) **Linear Pipeline（默认）**：`PLAN → (RETRIEVE) → (TOOL) → WRITE → GUARD`（P0/P1 均可用）  
...
- travel-ai 作为 eval 的一个 `target`：baseUrl + **`X-Eval-Token`**（已确认）
