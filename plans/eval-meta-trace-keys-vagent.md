...
**门控 / 安全 / 反思（分支依赖）**：`low_confidence`，`low_confidence_reasons`，`disabled_reason`，`eval_safety_rule_id`，`guardrail_triggered`，`reflection_outcome`，`reflection_reasons`；**P1-0**：短路路径可含 **`behavior`**、**`error_code`**（与评测 JSON 根级同值，便于 SSE 与 eval 对照）；条件满足时曾有 `retrieval_hit_ids`（不建议进 trace）。
