...
1. **P1-0（后续）**：SSE `meta` 与评测根级 `behavior`/`error_code` 对齐及 `rag/low_conf`×`SAFETY_QUERY_GATE` 说明见上表与本节；若还要把**成功路径**也强制写入 `meta.behavior=answer` 再单开任务。**门控距离/子串**已迁至 **`vagent.rag.gate.*`**（`RagPostRetrieveGateSettings`）；`vagent.eval.api.low-confidence-*` 仍绑定，作**兼容回退**。
...
| 安全短路 | `applySafetyShortCircuitMeta` + 根级 `error_code` | `meta` 含 `eval_safety_rule_id`、`low_confidence`/`low_confidence_reasons`（含 `SAFETY_QUERY_GATE` 时）、**`behavior`**、**`error_code`** |
