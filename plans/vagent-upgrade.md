...
| **D** | 工具治理 **P1-4** | 缺完整 **`ToolRegistry`**、配额/限流 | 入参/出参 schema（echo/ping）、**审计表 `mcp_tool_invocations`（D-5）** 已落地 |
...
- **P1-4 主链路工具治理（D1–D5）**：已落地 **D-1 最小 `ToolRegistry`**（仅 `echo/ping`，用于集中管理 schema key，避免校验器内 hardcode 工具名）；**仍无**完整 `ToolRegistry`（配额/限流等）；已有 `echo/ping` 入参 JSON Schema 与 `TOOL_SCHEMA_INVALID`（见 §P1-4 块引用），以及 `echo/ping` 出参 JSON Schema 与 `TOOL_RESULT_SCHEMA_INVALID`；**D-5** 审计表 `mcp_tool_invocations`（eval real / SSE，无 payload）；配额限流仍缺。
