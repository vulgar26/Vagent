...
| **D** | 工具治理 **P1-4** | 缺完整 **`ToolRegistry`** | 入参/出参 schema（echo/ping）、**审计（D-5）**、**进程内配额 `vagent.mcp.quota`（D-6）** 已落地 |
...
3. **P1-4（续）**：eval **桩**侧已可跑 **`p0_v0_tool_*`**，并具备 **桩输出 JSON Schema**、**熔断/超时**；**`tool_policy=real`** 与 **`HttpMcpClient`** 共用 **`vagent.mcp.tool-call-timeout`**。主链路 **入参/出参 schema（echo/ping）**、**审计表**、**进程内配额（`vagent.mcp.quota`）** 已落地；**完整 `ToolRegistry`、跨实例统一配额** 仍属缺口（见下节）。
...
- **P1-4 主链路工具治理（D1–D6）**：已落地 **D-1 最小 `ToolRegistry`**（仅 `echo/ping`，用于集中管理 schema key，避免校验器内 hardcode 工具名）；**仍无**完整 `ToolRegistry`（跨实例统一配额等）；已有 `echo/ping` 入参 JSON Schema 与 `TOOL_SCHEMA_INVALID`（见 §P1-4 块引用），以及 `echo/ping` 出参 JSON Schema 与 `TOOL_RESULT_SCHEMA_INVALID`；**D-5** 审计表 `mcp_tool_invocations`（eval real / SSE，无 payload）；**D-6** 进程内固定窗口配额（`vagent.mcp.quota`，`TOOL_RATE_LIMITED`，在入参 schema 通过后、`callTool` 前扣减；多实例各算各的）。
