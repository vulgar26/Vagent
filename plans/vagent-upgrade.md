...
3. **P1-4（续）**：eval **桩**侧已可跑 **`p0_v0_tool_*`**，并具备 **桩输出 JSON Schema**、**熔断/超时**；**`tool_policy=real`** 与 **`HttpMcpClient`** 共用 **`vagent.mcp.tool-call-timeout`**。文档原意的 **`ToolRegistry`、主链路工具入参/出参 schema、审计/限流** 仍属缺口（见下节）。
...
- **评测 `POST /api/v1/eval/chat`**：`EvalChatController`；`capabilities`；与 **`KnowledgeRetrieveService#searchForRag`** 联动；**`retrieval_hit_id_hashes[]`**（`buildRetrievalHitIdHashes` / HMAC、`k_case` 派生与 `eval-upgrade` 口径一致）、**`canonical_hit_id_scheme`**、候选截断相关 **`meta`**。
...
- **Quote-only**：`EvalQuoteOnlyGuard`；eval 与可选 **SSE 全文缓冲**（**`GuardrailsProperties.QuoteOnly#applyToSseStream`** + **`LlmSseStreamingBridge`**）；语义 SSOT：**`plans/quote-only-guardrails.md`**。
...
- **`evidenceMap[]`（P1-5）**：`EvalChatResponse.GuardrailsFlag` 含 **`evidenceMap` 布尔位**，**无**服务端构造的 **`evidenceMap[]` 数组**返回逻辑。
...
- **evidence map**：评测接口 `POST /api/v1/eval/chat` 响应增加 `evidenceMap[]`，由服务端从“回答 + sources”生成映射（禁止让 LLM 编造 sourceIds）。**当前代码**：`capabilities.guardrails` 仅有 **`evidenceMap` 布尔占位**，**无** `evidenceMap[]` 数组构造与返回。
- **quote-only**：对带 `guardrail/quote_only` 标签的 case，Vagent 以配置/模式启用“只允许引用内数字/关键实体”的约束；违规则降级为 `deny|clarify` 并打 `error_code`。**当前代码**：已实现 **`EvalQuoteOnlyGuard`**（eval 与可选 SSE **`apply-to-sse-stream`**）；语义与档位 SSOT 为 **`plans/quote-only-guardrails.md`**。
