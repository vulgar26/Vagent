...
- **P1-4 主链路工具治理（D1–D4）**：已落地 **D-1 最小 `ToolRegistry`**（仅 `echo/ping`，用于集中管理 schema key，避免校验器内 hardcode 工具名）；**仍无**完整 `ToolRegistry`（版本/resultSchema hash/审计/配额）；已有 `echo/ping` 入参 JSON Schema 与 `TOOL_SCHEMA_INVALID`（见 §P1-4 块引用），以及 `echo/ping` 出参 JSON Schema 与 `TOOL_RESULT_SCHEMA_INVALID`；审计表 / 配额限流仍缺。
...
- **输出校验失败（tool result）**：
  - `tool.outcome=error`，`meta.tool_error_code=TOOL_RESULT_SCHEMA_INVALID`（已在 SSOT `EvalErrorCodes` 预留，后续接 `ToolRegistry/resultSchema` 时落地校验）
