...
> **大白话（和 quote-only 最严档一起开时）**：**reflection → quote-only（含可选「数字必须对上证据表」）→ 再检查 `requires_citations`**。quote-only 里最严那档如果已经抽过 **`evidence_map`**，后面 **`requires_citations`** 会**直接复用那张表**，不重复抽。若仍交不出表，根上仍是 **`EVIDENCE_NOT_SUPPORTED`**，同时 **`meta`** 里会带 **`reflection_outcome`/`reflection_reasons`**（如 **`EVIDENCE_MAP_EMPTY`**）方便对账；详见 **`plans/quote-only-guardrails.md`**「大白话速读」。
