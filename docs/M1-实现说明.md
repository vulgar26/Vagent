...
1. `schema-core.sql`（及生产环境另见 `schema-vector.sql`）+ `application.yml` —— PostgreSQL DDL、数据源、`spring.sql.init`、`mybatis-plus`、`vagent.security.jwt`。  
...
flowchart TD
    ...
    Y --> SQL["spring.sql.init<br/>schema-core + schema-vector（M2）"]
    Y --> MP["mybatis-plus.*"]
...
| 文件 / 配置项 | 作用 |
|---------------|------|
| `schema-core.sql` | `users`、`conversations` 的 **PostgreSQL** DDL；**test profile** 下由 H2 仅执行本文件。 |
| `schema-vector.sql`（M2） | pgvector 扩展与 `kb_*` 表；勿加入仅 H2 的 test 初始化。 |
...
| `spring.sql.init.*` | 开发期执行 DDL（主配置含 `schema-core` + `schema-vector`）；**生产建议 `mode: never` 并改用 Flyway**。 |
