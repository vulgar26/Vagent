...
1. `src/main/resources/db/migration/`（生产/本地 PostgreSQL Flyway 迁移）与 `schema-core.sql`（test profile 轻量初始化）+ `application.yml` —— 数据源、Flyway、`mybatis-plus`、`vagent.security.jwt`。  
...
flowchart TD
    ...
    Y --> FW["Flyway<br/>db/migration（PG）"]
    Y --> MP["mybatis-plus.*"]
...
| 文件 / 配置项 | 作用 |
|---------------|------|
| `db/migration/` | Flyway 迁移脚本；生产/本地 PostgreSQL 的主要 DDL 来源。 |
| `schema-core.sql` | `users`、`conversations` 等核心表的轻量 DDL；**test profile** 下由 H2 执行。 |
| `schema-vector.sql`（M2） | pgvector 扩展与 `kb_*` 表的手工/历史参考；勿加入仅 H2 的 test 初始化。 |
| `application.yml` | 默认 **PostgreSQL** 连接；`spring.flyway.enabled=true`，`spring.sql.init.mode=never`，避免重复建表。 |
| `application-test.yml` | **仅 test profile**：内存 H2（PostgreSQL 兼容模式）+ 仅 `schema-core.sql`。 |
...
| `spring.flyway.*` | PostgreSQL 环境的迁移配置，脚本位于 `db/migration/`。 |
| `spring.sql.init.*` | 测试 profile 用于 H2 轻量初始化；主配置保持 `mode: never`。 |
...
| M3 | SSE 流式、`noop`/`fake-stream` `LlmClient`、任务取消（详见 [M3-实现说明.md](M3-实现说明.md)）。 |
