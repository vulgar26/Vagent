...
- **与主链路（阶段顺序）**：`travel-ai-planner` 已实现 **评测 stub 与 SSE `TravelAgent` 同为** `PLAN → RETRIEVE → TOOL → GUARD → WRITE`（`meta.stage_order` 与 `EvalLinearAgentPipeline` 一致；短路路径可为子序列如 `PLAN→GUARD`）。下文 §「控制流限制」若仍写 `…tool→write→guard` 为**文档叙述习惯**，实现与评测均以 **门控（GUARD）在流式写作（WRITE）之前** 为准。
...
> **实现真源**：`D:\Projects\travel-ai-planner`（`EvalChatController`、`EvalGatewayAuthFilter`、`AppEvalProperties` / `application.yml` 中 `app.eval.*`）。
