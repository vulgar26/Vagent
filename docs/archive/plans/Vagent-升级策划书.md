...
| MCP 工具 | **U6** MCP Client 与联调入口；**U7** 主链路显式工具意图 + 白名单受控子集，默认关闭 | **U6/U7** | 独立进程 Server；非完整工具治理平台 |
...
- **Flyway**：本仓库已引入 Flyway，生产/本地 PostgreSQL 以 `db/migration` 为准；`schema-*.sql` 主要服务测试与手工参考。
...
- 将 MCP 从“联调入口”升级为可用于主对话链路的能力：在 **RAG 编排**中按显式工具意图触发工具调用，并将结果并入 Prompt 上下文（对齐策划书 §3 的“检索聚合 KB + 工具侧上下文”表述）。当前是受控子集，默认关闭，不等同于完整生产级工具平台。
