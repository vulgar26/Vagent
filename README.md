...
- **对话与 RAG**：多轮 `messages` 持久化；流式入口拼历史、检索片段与系统提示后调用 **`LlmClient`**；SSE 首包 **`meta`** 含 **`taskId`**、**`branch`**（如 RAG / 寒暄 / 澄清）、**`hitCount`** 等；RAG 有检索 trace 时另有 **`canonical_hit_id_scheme`**、**`retrieval_candidate_*`**、**`retrieval_hit_id_hashes`**、**`retrieval_membership_*`**、**`chat_stream_channel`**（哈希需 **`vagent.rag.sse-membership-hmac-secret`**，与评测 **E7** 的 token 材料不同源，见 **`plans/vagent-upgrade.md`** 缺口 **A / A-2**）。
